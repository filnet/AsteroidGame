...
namespace GameLibrary.Component.Camera
{
    ...
    /// </summary>
    public class DefaultCamera : GameComponent, CameraComponent
    {
        public enum Actions
        {
        ...
        private const float DEFAULT_SPEED_ROTATION = MathHelper.Pi / 1000;
        ...
        /// </summary>
        public DefaultCamera(Game game) : base(game)
        {
            camera = new Camera();
            ...
            */
            ...
            if (v.X != 0 || v.Y != 0)
            {
                if (triggers.Left > 0)
                {
                    ...
                    if (v.X != 0)
                    {
                        RotateSmoothly(-v.X * elapsed * 500, 0, 0);
                    }
                }
                else
                {
                    RotateSmoothly(-v.X * elapsed * 500, v.Y * elapsed * 500, 0);
                }
            }
    ...
    internal sealed class MouseCage
    {
        ...
        internal void Update(Rectangle clientBounds)
        {
            ...
            */
            ...
            if ((Math.Abs(centerX - currentMouseState.X) >= clientBounds.Width / 4) || (Math.Abs(centerY - currentMouseState.Y) >= clientBounds.Height / 4))
            {
                // see https://github.com/MonoGame/MonoGame/issues/6262#issuecomment-417870908https://github.com/MonoGame/MonoGame/issues/6262#issuecomment-417870908
    ...
    internal sealed class MouseFilter
    {
        ...
        {
            mouseSmoothingCache = new Vector2[DefaultCamera.MOUSE_SMOOTHING_CACHE_SIZE];
        ...
        {
            mouseMovement[mouseIndex].X = smoothedMouseMovement.X;
            mouseMovement[mouseIndex].Y = smoothedMouseMovement.Y;
