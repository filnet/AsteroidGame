...
jQuery(document).ready(function() {
    var gem_book;
    var gem_page;
    gem_book = location.pathname.split('/')[2];
    gem_page = location.pathname.split('/')[3];

    var load_book_page = function(book, page) {
                jQuery('#book_page').load(Drupal.settings.gpugems.base +'/'+ book + '/' + page, function() {                                                                                                                          
                        jQuery('#book_page img').each(function(index, item) {                                                                                                                                                                   
                                $item = jQuery(item);                                                                                                                                                                                           
                                $item.attr('src', Drupal.settings.gpugems.base + '/' + book + '/' + $item.attr('src'));
                        });
                        doc_path = '/gpugems/' + book + '/' + page;
                        history.pushState({}, '', doc_path);
                        _gaq.push(['_trackPageview', doc_path]);

                }); 
    }

    var load_page_from_link = function(e) {
                e.preventDefault();                                                                                                                                                                                                             
                var $this = jQuery(this);                                                                                                                                                                                                       
                var book = window.location.pathname.split('/')[2];                                                                                                                                                                              
                var page = $this.attr('href').split('/').pop();
                load_book_page(book, page);                                                                                                                                                                                                     
        }

    jQuery('#book_page').delegate('li a', 'click', load_page_from_link); 
    jQuery('#book_switch a.btn-primary').click(function(e) {
        e.preventDefault();
            var gem_book = jQuery(this).attr('href').split('/')[2];
                var gem_page = jQuery(this).attr('href').split('/')[3];
        load_book_page(gem_book, gem_page);
    });

    if(typeof(gem_book) == 'undefined' || typeof(gem_page) == 'undefined') {
        jQuery('#book_switch .btn-primary').first().click();
    } else {
        load_book_page(gem_book, gem_page);
    }
});
