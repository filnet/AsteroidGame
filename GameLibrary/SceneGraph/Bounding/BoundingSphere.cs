...
namespace GameLibrary.SceneGraph.Bounding
{
    public class BoundingSphere : BoundingVolume
    {
        ...
        {
            BoundingSphere rVal;
            if (store == null || store.GetBoundingType() != BoundingType.Sphere)
            {
                rVal = new BoundingSphere();
            }
            else
            {
                rVal = store as BoundingSphere;
            }

            rVal.Center = Center * scale;
            rVal.Center = Vector3.Transform(rVal.Center, rotation);
            rVal.Center += translation;
            rVal.Radius = System.Math.Abs(GetMaxAxis(scale) * Radius) + EPSILON;
        ...
        public override BoundingVolume Transform(Matrix m, BoundingVolume store)
        {
            ...
            {
                return Transform(scale, rotation, translation, store);
            }
            return null;
        }
        ...
        {
            float x = System.Math.Abs(scale.X);
            float y = System.Math.Abs(scale.Y);
            float z = System.Math.Abs(scale.Z);
